...
async def test_what_if_skips_causal_events():
    ...
    setup_events = [
        ...
        {"event_type": "DocumentUploadRequested", "event_version": 1, "payload": {"application_id": app_id}},
        {"event_type": "DocumentUploaded", "event_version": 1, "payload": {"application_id": app_id, "document_id": "doc-1"}},
        ...
        {"event_type": "ComplianceCheckRequested", "event_version": 1, "payload": {"application_id": app_id}},
        {"event_type": "DecisionRequested", "event_version": 1, "payload": {"application_id": app_id}},
    ]
    expected_version = -1
    for event in setup_events:
        positions = await store.append(stream, [event], expected_version=expected_version)
        expected_version = positions[-1]

    # Append DecisionGenerated with causation_id of DecisionRequested
    events = await store.load_stream(stream)
    decision_request_id = events[-1]["event_id"]
    ...
    }}], expected_version=expected_version, causation_id=decision_request_id)
    ...
    result = await run_what_if(store, app_id, branch_global_position=6, counterfactual_events=[cf_event])
